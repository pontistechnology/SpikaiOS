...
class DatabaseService {
    ...
    init(coreDataStack: CoreDataStack, phoneNumberParser: PhoneNumberParser) {
...
//                        let timer = ParkBenchTimer()
                        do {
                            try store.enumerateContacts(with: request, usingBlock: { (contact, stopPointer) in
                                ...
                                //self.phoneNumberParser.phoneNumberKit.parse(contact.phoneNumbers.map { $0.value.stringValue })
//                                    .map {  self.phoneNumberParser.phoneNumberKit.format($0, toType: .e164, withPrefix: true) }
